...
public class SummationOfContiniousElementInArray {
	...
	static void findSubArray(int[] inputArray, int inputNumber) {
		...
		for (int i = 1; i < inputArray.length; i++) { // Adding inputArray[i] to the current 'sum' sum = sum +
														// inputArray[i]; //If sum is greater than inputNumber then
														// following loop is executed until //sum becomes either smaller
														// than or equal to inputNumber while(sum > inputNumber && start
														// <= i-1)
			{
